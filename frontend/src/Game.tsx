...
export default function Game({ username }: { username: string }) {
    ...
    return <>
        ...
        {questions.length && questions.length === question && <>
            <Text>Your score: {score} / {questions.length}</Text>
            <Text>Answers</Text>
            {questions.map((item, i) => <Text key={i}>Country {item[0]} has area of {item[1].toLocaleString('en-US')} <span>km<sup>2</sup></span></Text>)}
        </>}
